const content = {
  title: "Acerca de Nosotros",
  subtitle: "Conocer al equipo",
  people: [
      {
          name: "Dra. Cynthia Villacis MD",
          title: "Médico de Familia y Life Coach",
          image: "brainz.jpg",
          bio: ["La Dra. Cynthia Villacis abrió Health Connections Direct Primary Care en 2017 para brindarles a los pacientes el tiempo y la atención que necesitan sin las limitaciones típicas de una práctica basada en seguros.",

          "Habiendo lidiado personalmente con la obesidad, la presión arterial, la diabetes y la ansiedad, comprende cómo los problemas de salud pueden llevar a algunos médicos a hacer suposiciones sobre los intereses y habilidades de un paciente. En su práctica, se asegura de que sus pacientes nunca se enfrenten a tales juicios.",
          
          "También ha practicado la medicina de la adicción desde 2015, lo que le permite una comprensión única cuando trabaja con pacientes afectados por la adicción.",
          
          "Ella cree en la creación de un entorno en el que los pacientes se sientan abiertos sobre sus vidas, permitiéndoles ayudarlos a buscar una mejor salud física, espiritual y emocional, personalizada según sus necesidades.",
          
          "Residente del norte de Kentucky desde 1999, el Dr. Villacis disfruta de actividades al aire libre como kayak, natación, mochilero y buceo. Alienta a sus pacientes a mantenerse físicamente activos y explorar el mundo, independientemente de su tamaño, mientras trabajan para mejorar su salud."],
          educationTitle: "Educación",
          education: [{ "Escuela Secundaria": "Sycamore, Montgomery, OH" },
                      {"De Licenciatura": "Oberlin College, Oberlin, OH"},
                      {"Escuela de Medicina": "Wright State, Dayton, OH  - 1999"},
                      {"Residencia": "St. Elizabeth’s, Edgewood, KY - 2002"},],
          certificationsTitle: "Certificaciones",    
          certifications: ["Médico de familia certificado por la junta",
                          "Life Coach certificado a través de Life Coach School"],
          membershipsTitle: "Membresías",
          memberships: ["American Academy of Family Physicians",
                      "Northern Kentucky Medical Society",
                      "American Society of Addiction Medicine"],
      },
      {
          name: "Danielle",
          title: "Asistente Medica, CCMA",
          image: "brainz.jpg",
          bio: ["Danielle se unió al equipo de Health Connections en 2019, atraída por el enfoque centrado en el paciente de la práctica. Las visitas más largas al consultorio permiten pasar más tiempo con los pacientes durante sus citas, fomentando una relación más cercana y personalizada. El ambiente íntimo de una práctica más pequeña asegura que cada paciente sea bien conocido y atendido.",
              "Fuera del trabajo, le gusta hacer manualidades y animar los partidos de fútbol de su hijo. Danielle ha estado casada por 12 años y es una orgullosa dueña de dos gatos, Milo y Lulu. La familia es claramente una parte apreciada de su vida.",
              "Actualmente aceptando el desafío de aprender español como segundo idioma, Danielle está comprometida con el crecimiento personal y la participación comunitaria.",
              "El verano encuentra a la familia en la piscina o planeando viajes a la playa, disfrutando de la alegría de la relajación y la conexión."],
      },
  ]
}

export default content;